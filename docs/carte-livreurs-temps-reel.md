...
La page **Carte des livreurs** (`/drivers/map`) affiche les positions GPS en temps réel des chauffeurs dont la **tournée est marquée « en cours »** côté backend. Elle ne s’appuie **pas** sur deux APIs distinctes (drivers / deliveries) : une seule API **deliveries** fournit la liste des livraisons et leur statut ; le détail d’une livraison (dont le nom du chauffeur) est chargé à la demande.
...
1. **Période**  
   La page utilise la plage de dates du filtre global (`dateRangeState`). Si aucune période n’est choisie → message « Période requise ».

2. **Chargement des tournées**  
   Un seul appel HTTP :
...
4. **Affichage**  
   - Si **aucune** livraison n’a le statut « en cours » → message :  
     **« Aucune tournée en cours sur cette période. Les positions des livreurs s'affichent lorsqu'une tournée est démarrée. »**
   - Sinon : la carte s’affiche, et le client se connecte à SignalR pour recevoir les positions des livraisons dans `inProgressIds`.
...
**En pratique :** si tu as déjà la page carte ouverte quand le chauffeur démarre le suivi, il faut **rafraîchir la page** (ou modifier la période) pour que le front recharge la liste et affiche la tournée + la carte.
