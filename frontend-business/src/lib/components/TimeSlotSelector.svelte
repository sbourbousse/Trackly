<script lang="ts">
	...
	function selectSlotInHours(hours: number) {
		...
		// Vérifier si on passe au lendemain
		...
		if (isToday && onDateChange) {
			...
			// Si la date future est différente d'aujourd'hui, mettre à jour la date
			if (futureDateCalendar.year !== todayRef.year || 
