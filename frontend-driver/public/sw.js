...
self.addEventListener('fetch', (event) => {
  ...
  // Ne pas intercepter les requêtes vers l'API ou SignalR en développement
  // Laisser passer toutes les requêtes réseau pour l'API
  if (url.pathname.startsWith('/api/') || url.pathname.startsWith('/hubs/')) {
    // Stratégie network-only pour les requêtes API
    event.respondWith(
      fetch(request).catch((error) => {
        // Seulement retourner 503 si vraiment hors ligne
        if (!navigator.onLine) {
          return new Response(
